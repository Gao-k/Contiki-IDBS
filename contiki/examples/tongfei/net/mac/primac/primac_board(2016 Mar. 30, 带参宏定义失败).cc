...
#define PRI_CCA_CHECK_TIME(rs)                     (PRIMAC_CONF_CCA_CHECK_TIME)
#else
#define PRI_CCA_CHECK_TIME(rs)                     (rs / 8192) // 2000
#endif
...
#endif
*/
#define PRI_CCA_SLEEP_TIME(rs)	(rs>8000)? (rs/2000):((rs/2000)+1)
...
#if IS_COOJA_SIM
	#define PRI_TM(rs)			(rs/455)//=72, 0.00219s
#else
	#define PRI_TM(rs)			0//0//(RTIMER_SECOND/455)//=72, 0.00219s
#endif
/*
* totally 20 ms
...
/**********************************************/
/*Mar. 29, 2016: replace all RTIMER_SECOND with rs*/
/**********************************************/
#define WAKEUP_ADVANCE_TIME(rs)		(rs/100) // 10 ms
#define	R_END_GUARD_TIME(rs)	WAKEUP_ADVANCE_TIME(rs)

#define DURGRADE(rs)	(rs/1023) //=364, 0.011s (1/91) propagation time of RTS (durRTS in the paper)
#define DURRTS(rs)		(rs/1023) //=364, 0.011s (1/91) propagation time of RTS (durRTS in the paper)
#define DURCTS(rs)		(rs/1023) //=364, 0.011s (durCTS)
//#define DURACK(rs)		(rs/1203) //=364, 0.011s
#define DURDATA(rs)		(rs/46) //=819, 28,1170,0.036s (1/23=, 0.043s)
#define CW(rs)			(rs/512) //=520, 0.016s 
#define SIGMA(rs)		(rs/1000) // equivalent to 1 ms =1/1000 s
//#define MIN_CW_POW	2
//#define UCAST_CTS_CW			(RTIMER_SECOND/256) // 32768/256= 128 
#define DIFS(rs)		(rs/100)//=327, 0.01s
#define SIFS(rs)		(rs/200)//=163, 0.005s
#define GUARDTIME(rs)	(rs/100)//=163, 0.005s

#define WAIT_TO_SEND_RTS(rs)			(DIFS(rs)+(random_rand() % CW(rs))*SIGMA(rs)) //time duration for waiting to send RTS
#define WAIT_TO_SEND_GRADE(rs)		WAIT_TO_SEND_RTS(rs) //time duration for waiting to send the GRADE message
...
#define WAIT_TO_RCV_RTS(rs)			(DIFS(rs)+CW(rs)*SIGMA(rs)+DURRTS(rs)+ WAKEUP_ADVANCE_TIME(rs)+R_END_GUARD_TIME(rs) + GUARDTIME(rs)) //time duration for waiting to receiving RTS

//#define WAIT_TO_REG_BCAST_CTS(rs)			(SIFS(rs)+(random_rand() % CW(rs))*SIGMA(rs)) //time duration for waiting to contend for replying with CTS gregularly
...
#define	WAIT_TO_UCAST_CTS(rs)			(SIFS(rs)) //time duration for waiting to unicast CTS

//#define WAIT_TO_RCV_CTS			(SIFS+CW*SIGMA+DURCTS+GUARDTIME+PRI_CHECK_TIME_TX)
#define WAIT_TO_RCV_CTS(rs)			(SIFS(rs)+CW(rs)*SIGMA(rs)+DURCTS(rs)+GUARDTIME(rs))

#define WAIT_TO_RCV_DATA(rs)		(SIFS(rs)+DURDATA(rs)+GUARDTIME(rs))//+PRI_CHECK_TIME_TX)
...
#define DURSLOT(rs)		((uint32_t)(DIFS(rs)+2*CW(rs)*SIGMA(rs)+3*SIFS(rs)+DURRTS(rs)+DURCTS(rs)+DURDATA(rs)+R_END_GUARD_TIME(rs)))//+DURACK)) //3367,duration of R or T (durR=durT)

//Mar. 29, 2016
#define MAX_CLOCK_DRIFT(rs)		(WAKEUP_ADVANCE_TIME(rs)/2)// 3 //if the clock drift amount is larger than this number, then adjust the cycle period.
#define MAX_CYCLE_COUNTER(rs)	(((int32_t)(MAX_CLOCK_DRIFT(rs)))<<8)//(((uint32_t)(2)<<(sizeof(int16_t)*8-1))-1)// (uint32_t)5//   /*if the two boards sync with each other very well, then we need to set this MAX_CYCLE_COUNTER*/
...
#define DURSLEEP(rs)	((uint32_t)(SF+1)*(uint32_t)DURSLOT(rs)) //37037, duration of sleep (T_S in the paper, the 'T' state period is included).
#define DURCYCLE(rs)	((uint32_t)(SF+2)*(uint32_t)DURSLOT(rs)) //cycle time duration, (T_cycle in the paper).

#define RTMAXTIMER	(((uint32_t)1<<(sizeof(rtimer_clock_t)*8))-1)//((uint32_t)(RTIMER_SECOND-1)) // ((uint32_t)(RTIMER_SECOND-1)*2)
...
//Mar. 29, 2016
//#define MAX_CLOCK_DRIFT		(WAKEUP_ADVANCE_TIME/2)// 3 //if the clock drift amount is larger than this number, then adjust the cycle period.
...
//DATA Transmission
	...
	#define	CYCLES_PER_PACKET(rs)		(((DATA_INTERVAL+random_rand()%10)*(uint32_t)(rs))/DURCYCLE(rs))/*unit: cycles/packet, send one packet every such cycles (30 s)*/
	#ifndef RETX_LIMIT
...
struct pri_attribute_t //attributes maintained by primac for scheduling
{
	...
	uint32_t	sink_rs; /*the RTIMER_SECOND of its parent node*/
...
/*---------------------------------------------------------------------------*/
static rtimer_clock_t rtimer_normalization(rtimer_clock_t time_duration)
{
	return (rtimer_clock_t)((uint32_t)time_duration*pri_attribute.sink_rs/(uint32_t)RTIMER_SECOND);
}
...
send_grade() //只在pri_powercycle里调用
{
	...
	if(NETSTACK_FRAMER.create() >= 0) 
	{
	...
	#if CROSS_PLATFORM_SUPPORT
		//grade_msg.durslot = DURSLOT(pri_attribute.sink_rs);
		grade_msg.sink_rs = pri_attribute.sink_rs;
		...
		{
			data_ptr->state_dur = rtimer_normalization(now - pri_attribute.ADCSC_state_start);
		}else
		{
	#endif
		data_ptr->state_dur = rtimer_normalization(now - pri_attribute.state_start); 
	#if ADCSC_SUPPORT
		}
	#endif
...
send_rts(const int8_t not_for_data) //只在pri_powercycle里调用
{
	...
	#if CROSS_PLATFORM_SUPPORT
		//rts_msg.durslot = DURSLOT(pri_attribute.sink_rs);
		rts_msg.sink_rs = pri_attribute.sink_rs;
		//printf("arch second = %u.\n",grade_msg.sink_rs);
		//printf("sending, size of grade_msg_t: %d.\n",sizeof(struct grade_msg_t));
		//printf("send rts.\n");
	#endif
...
send_cts()//(const linkaddr_t* dest_addr_ptr) //只在pri_powercycle里调用
{
	...
	if(NETSTACK_FRAMER.create() >= 0) 
	{
	...
	#endif
	#if CROSS_PLATFORM_SUPPORT
		//cts_msg.durslot = DURSLOT(pri_attribute.sink_rs);
		cts_msg.sink_rs = pri_attribute.sink_rs;
		//printf("arch second = %u.\n",grade_msg.sink_rs);
		//printf("sending, size of grade_msg_t: %d.\n",sizeof(struct grade_msg_t));
	#endif
		...
		if(pri_attribute.ADCSC_unregular_wakeup == 1)//it's an unregular wakeup
		{
			data_ptr->state_dur = rtimer_normalization(now - pri_attribute.ADCSC_state_start);// + WAKEUP_ADVANCE_TIME);
		}else
		{
	#endif
		data_ptr->state_dur = rtimer_normalization(now - (pri_attribute.state_start + WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)));
	#if ADCSC_SUPPORT
		}
	#endif
...
pri_schedule_powercycle_fixed(struct rtimer *r_t, rtimer_clock_t fixed_time)
{
	...
	if(primac_is_on)
	{
	#if 0/*Do not set it to 1, otherwise it will affect the schedule*/
...
find_true_offset(int16_t time_offset)
{
	...
	for(s_tb=list_head(pri_neighbor_child_list); s_tb!=NULL; s_tb=s_tb->next)
	{
		...
		if((time_offset + s_tb->clock_drift_to_me) >= ((int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)))
		{
			//temp_offset = ((int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)) + abs(s_tb->cd_difference_to_last) - s_tb->clock_drift_to_me;
			temp_offset = (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs) + 5 - s_tb->clock_drift_to_me;
			//printf("1.temp_to:%d, cd: %d.\n",temp_offset, s_tb->clock_drift_to_me);
		}else if((time_offset + s_tb->clock_drift_to_me) <= -((int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)))
		{
			//temp_offset = - ((int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)) - abs(s_tb->cd_difference_to_last) - s_tb->clock_drift_to_me;
			temp_offset = - (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs) - 5 - s_tb->clock_drift_to_me;
	...
	while(1) 
	{
		...
		)
		{
			...
			)
			{
				...
				t0 = pri_attribute.state_start+WAIT_TO_SEND_GRADE(pri_attribute.sink_rs); //wait for DIFS + rand(CW) 
			#if 1
			...
			else if((!pri_attribute.fail_in_R) && (pri_attribute.grade>=0) && 
				(list_head(rdc_pri_buf_list)!=NULL||pri_attribute.periodic_rts_not_for_data_flag==1 || pri_attribute.clock_drift_rts_not_for_data_flag==1))
			{
				...
				{
					t0 = pri_attribute.ADCSC_state_start+ WAIT_TO_SEND_RTS(pri_attribute.sink_rs);
						//DIFS+(random_rand()%(2^pri_attribute.cw_pow_counter))*SIGMA;//
				}else 
				{
				#endif
				t0 = pri_attribute.state_start+ WAIT_TO_SEND_RTS(pri_attribute.sink_rs); //等待DIFS 和rand(CW)  时间
					//DIFS+(random_rand()%(2^pri_attribute.cw_pow_counter))*SIGMA;//
				#if ADCSC_SUPPORT
				}
				#endif
				...
				*/
				...
				}else //able to send RTS
				{
					...
					if(send_rts(pri_attribute.periodic_rts_not_for_data_flag|pri_attribute.clock_drift_rts_not_for_data_flag) == RADIO_TX_OK)
					{
						if(pri_attribute.pri_type == PRI_SENSOR)
						{
							...
							*/
							...
							pri_schedule_powercycle_fixed(r_t, now+WAIT_TO_RCV_CTS(pri_attribute.sink_rs)); //set timer for waiting for CTS
							PT_YIELD(&pt);
							pri_control.wait_cts_timer_over = 1;
						#endif
						#if 0
							t0 = RTIMER_NOW()+WAIT_TO_RCV_CTS(pri_attribute.sink_rs);
							while(RTIMER_CLOCK_LT(RTIMER_NOW(),t0))//(rand_cw--) 
							{
								if(NETSTACK_RADIO.channel_clear() == 0) 
								{
							...
							{
						#endif
							if(pri_control.rcvd_cts && !pri_control.queue_is_full)// && !pri_control.channel_is_busy)
							{
								
								if(!pri_attribute.periodic_rts_not_for_data_flag && !pri_attribute.clock_drift_rts_not_for_data_flag)
								...
								//printf("cts\n");/*mar.19,2016*/
									...
									now = RTIMER_NOW(); 
									t0 = now+SIFS(pri_attribute.sink_rs);
    				...
    				if(list_head(rdc_pri_buf_list)!=NULL)
    				{
       					if(pri_attribute.retx_limit > 0)
       					{
       						...
       						if(pri_attribute.retx_limit == 0)
       						{
       							...
       							list_remove(rdc_pri_buf_list,buf_elem);
       							memb_free(&rdc_pri_buf_memb,buf_elem);//free memory
       							// printf("3. retx limit drop data, Length:%d\n",list_length(rdc_pri_buf_list));
       						}
       					}else if(pri_attribute.retx_limit == 0)
       					{
       						pri_attribute.retx_limit = RETX_LIMIT;
       					}
			...
			if(pri_attribute.grade >= 0) /*update the related varialbes, do synchroization*/
			{
				...
				if(pri_attribute.pri_type == PRI_SENSOR && 
					((pri_attribute.to_broadcast_rts && pri_attribute.sender_num_in_receivers >= MAX_CHILD_NUM && !pri_attribute.has_receiver)//list_head(pri_neighbor_parent_list)==NULL)
				...
				#endif
					//Mar. 29, 2016
					pri_attribute.sink_rs = RTIMER_SECOND;
				...
				#if DATA_TX_SUPPORT
					pri_attribute.data_cycles_counter = CYCLES_PER_PACKET(pri_attribute.sink_rs);
				#endif
					pri_attribute.fail_in_R = 0;
			...
			if(pri_attribute.ADCSC_unregular_wakeup==0)///current it's a regular waekup
			{
				...
				if(pri_attribute.pri_type == PRI_SENSOR)/*only sensor will do synchronization*/
				{
					if(pri_control.rcvd_cts)//??? pri_control.rcvd_cts : use this as a switch ? only after receiving a CTS
					{
						...
						/*******************************************************************************************************************/
						if(r_tb!=NULL && (abs(r_tb->clock_drift) >= (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)))// need to adjust clock: METHOD 1: adjust only when the threshold is reached
						/*******************************************************************************************************************/
						...
						{ 
							...
							{
								/*single side:*/
								time_offset = r_tb->clock_drift;// + MAX_CLOCK_DRIFT(pri_attribute.sink_rs); 
								/*double side:*/
								//time_offset = r_tb->clock_drift + (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs); 
								//printf("1.pos.to: %d.\n",time_offset);
								...
								/*double side:*/
								//r_tb->clock_drift = - (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs); //0;//update the clock drift of itself
							...
							else //if((r_tb->clock_drift - r_tb->cd_base)<0)//negative drift 
							{
								...
								r_tb->clock_drift -= time_offset;
								/*double side:*/
								//r_tb->clock_drift = (int16_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs);//0;//
					...
					if(!pri_attribute.to_broadcast_rts) /*update only when it's changed to unicast*/
					{
						...
						if(r_tb != NULL)
						{
							...
							{	
								...
								//PRT_FORSYNC("MAX cycle counter: %ld .\n",(int32_t)(MAX_CYCLE_COUNTER*MAX_CLOCK_DRIFT(pri_attribute.sink_rs))/abs(l_need_send_rts->clock_drift - l_need_send_rts->cd_base));
								if(r_tb->cycle_counter <= (int32_t)(MAX_CYCLE_COUNTER(pri_attribute.sink_rs)*MAX_CLOCK_DRIFT(pri_attribute.sink_rs))/abs(r_tb->clock_drift - r_tb->cd_base))
								{
								 	//PRT_FORSYNC("send RTS for clock drift: cc: %ld , possible ccth: %ld .\n",l_need_send_rts->cycle_counter,
								 	//	(MAX_CLOCK_DRIFT(pri_attribute.sink_rs)*l_need_send_rts->cycle_counter)/abs(l_need_send_rts->clock_drift- l_need_send_rts->cd_base));
									if(r_tb->cycle_counter >= ((int32_t)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)*r_tb->cycle_counter)/abs(r_tb->clock_drift- r_tb->cd_base))
		...
		)
		{
			...
			if(pri_attribute.grade>=0) //&& list_length(rdc_pri_buf_list) < MAX_DATA_QUEUE_BUFS)
			{
				...
				pri_control.channel_is_busy = 0;
				now = RTIMER_NOW();
				//t0 = RTIMER_NOW()+WAIT_TO_RCV_RTS(pri_attribute.sink_rs); 
				pri_control.wait_rts_timer_over = -1;
				pri_schedule_powercycle_fixed(r_t, now+WAIT_TO_RCV_RTS(pri_attribute.sink_rs)); //set timer for waiting for RTS
				PT_YIELD(&pt);
				//printf("rts\n");
				pri_control.wait_rts_timer_over = 1; 
			#endif
				...
				if(pri_control.rcvd_rts==1 && pri_attribute.rcvd_cts_after_rts==1 && !pri_control.channel_is_busy)
				{
					...
					if(pri_attribute.pri_type == PRI_SENSOR)//pri_control.rcvd_bro_rts)//received broadcasted RTS
					{
						...
						{
							t0 = RTIMER_NOW()+WAIT_TO_REG_BCAST_CTS(pri_attribute.sink_rs); //等待SIFS 和rand(CW)  时间
						...
						t0 = CW(pri_attribute.sink_rs)/(MAX_CHILD_NUM+1); //wait to reply CTS
						now = RTIMER_NOW();
						t0 = now + SIFS(pri_attribute.sink_rs) + (t0*list_length(pri_neighbor_child_list)+(random_rand() % t0))*SIGMA(pri_attribute.sink_rs);
						/*
						*	Use the following module to replace pri_cca() (which has been commented above);
						*	To reply with CTS
						*/
					...
					{
						//t0 = WAIT_TO_UCAST_CTS(pri_attribute.sink_rs); //等待DIFS 和rand(CW)  时间
						//printf("Unicast CTS. t0=%lu.\n",t0);
						t0=RTIMER_NOW()+WAIT_TO_UCAST_CTS(pri_attribute.sink_rs);
					...
					*/	
						pri_powercycle_turn_radio_on();
						if(send_cts() == RADIO_TX_OK && list_length(rdc_pri_buf_list) < MAX_DATA_QUEUE_BUFS)
						{
							pri_control.sent_cts = 1;
							if(!pri_control.rcvd_rts_not_for_data) //able to wait data
							{
							#if 1
								now = RTIMER_NOW();
								pri_control.wait_data_timer_over = -1;
								pri_schedule_powercycle_fixed(r_t, now+ WAIT_TO_RCV_DATA(pri_attribute.sink_rs));
								PT_YIELD(&pt);
								pri_control.wait_data_timer_over = 1;
							#endif
								...
								t0 = RTIMER_NOW()+WAIT_TO_RCV_DATA(pri_attribute.sink_rs);
								while(RTIMER_CLOCK_LT(RTIMER_NOW(),t0))//(rand_cw--) 
								{
									if(NETSTACK_RADIO.channel_clear() == 0) 
									{
		 ...
		 //		pri_attribute.ADCSC_state,pri_attribute.ADCSC_to_recv,pri_attribute.ADCSC_to_send);
			if(pri_attribute.ADCSC_state == 'R')/// currently it's in state 'R'
			{
				if(pri_attribute.pri_type == PRI_SINK && pri_attribute.ADCSC_to_recv == 2)
				{
					...
					t0 = DURSLOT(pri_attribute.sink_rs);
					now = RTIMER_NOW();
					t0 = t0 - rtimer_normalization(now - pri_attribute.ADCSC_state_start);
					pri_schedule_powercycle_fixed(r_t, now + t0);// - WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)); 
					PT_YIELD(&pt);//进程主动让出执行权
			...
			}else if (pri_attribute.ADCSC_state == 'S') /// in state 'T'
			{
				if(pri_attribute.ADCSC_to_recv)/// currently it's in T state && need to receive in sleeping period
				{
					...
					t0 = DURSLOT(pri_attribute.sink_rs)*3;
					now = RTIMER_NOW();
					t0 = t0 - rtimer_normalization(now - pri_attribute.ADCSC_state_start);
					pri_schedule_powercycle_fixed(r_t, now + t0);// - WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)); 
					PT_YIELD(&pt);
				}else if(pri_attribute.ADCSC_to_send)/// currently it's in T state && no need to wake up for receiving but just for transmitting
				{
					...
					}else
					{
						...
						t0 = DURSLOT(pri_attribute.sink_rs)*4;
						now = RTIMER_NOW();
						t0 = t0 - rtimer_normalization(now - pri_attribute.ADCSC_state_start);
						pri_schedule_powercycle_fixed(r_t, now + t0);// + WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)); 
						PT_YIELD(&pt);//进程主动让出执行权
				...
				}else///ready to enter regular wakeup
				{
					...
					t0 = DURSLOT(pri_attribute.sink_rs)*(pri_attribute.ADCSC_left_SF+1);
					now = RTIMER_NOW();
					t0 = t0 - rtimer_normalization(now - pri_attribute.ADCSC_state_start);
...
#if 0
					t0 = pri_attribute.ADCSC_state_start+DURSLOT(pri_attribute.sink_rs)*(pri_attribute.ADCSC_left_SF+1);
					if(DURSLOT(pri_attribute.sink_rs)*(pri_attribute.ADCSC_left_SF+1) > RTMAXTIMER)
					{
		...
		//		pri_attribute.state,pri_attribute.ADCSC_to_recv,pri_attribute.ADCSC_to_send);
			...
			if(pri_attribute.state == 'S' && pri_attribute.ADCSC_to_recv) /// currently it's in T state && need to receive in sleeping period
			{
				...
				t0 = DURSLOT(pri_attribute.sink_rs)*3;
				now = RTIMER_NOW();
				t0 = t0 - rtimer_normalization(now - pri_attribute.state_start);
				pri_schedule_powercycle_fixed(r_t, now + t0);// - WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)); 
				PT_YIELD(&pt);//进程主动让出执行权
			...
			}else if(pri_attribute.state == 'R' && pri_attribute.pri_type == PRI_SINK && pri_attribute.ADCSC_to_recv == 2)   
			{
				...
				t0 = DURSLOT(pri_attribute.sink_rs);
				now = RTIMER_NOW();
				t0 = t0 - rtimer_normalization(now - pri_attribute.state_start) + WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs);
				pri_schedule_powercycle_fixed(r_t, now + t0);
				PT_YIELD(&pt);//进程主动让出执行权
				//pri_attribute.ADCSC_left_SF = pri_attribute.ADCSC_left_SF - 1;
			}else /// no need to receive or send during the sleep period
			{
				...
				t0 = pri_state_period();
				now = RTIMER_NOW();
				t0 = t0 - rtimer_normalization(now - pri_attribute.state_start);// + time_offset + pri_attribute.drift_per_cycle;
...
*/
	...
	if(pri_attribute.state == 'R' )
	{
		...
		*/
		return (DURSLOT(pri_attribute.sink_rs) + WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs)); 
	}else if (pri_attribute.state == 'S')
	{
		//printf("DURSLEEP=%lu\n",DURSLEEP);
		/*when considering wakeup advance time, */
		return (DURSLEEP(pri_attribute.sink_rs) - WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs));//the T period is included
	}else
	{
...
static void print_definition(void)
{
...
#endif
#if 0
	float f = ((float)DURSLOT(pri_attribute.sink_rs)*1.0)/((float)RTIMER_SECOND*1.0);
	PRT_DEF("DURSLOT = %lu.",DURSLOT(pri_attribute.sink_rs));
	PRT_DEF("(%d.%04u s).\n",(int)f,(unsigned)((f-floor(f))*10000));
	PRT_DEF("DURSLEEP = %lu.\n",DURSLEEP(pri_attribute.sink_rs));
	f = ((float)DURCYCLE(pri_attribute.sink_rs)*1.0)/((float)RTIMER_SECOND*1.0);
	PRT_DEF("DURCYCLE = %lu .",DURCYCLE(pri_attribute.sink_rs));
	PRT_DEF("(%d.%04u s).\n",(int)f,(unsigned)((f-floor(f))*10000));
	f = ((float)MAX_CLOCK_DRIFT(pri_attribute.sink_rs)*1.0000)/((float)RTIMER_SECOND*1.0000);
	PRT_DEF("MAX_CLOCK_DRIFT = WAKEUP_ADVANCE_TIME = %d (%d.%04u s).\n",(int16_t)WAKEUP_ADVANCE_TIME(pri_attribute.sink_rs),(int)f,(unsigned)((f-floor(f))*10000));
#endif
	PRT_DEF("DURSLOT = %lu.",DURSLOT(pri_attribute.sink_rs));
	PRT_DEF("DURSLEEP = %lu.\n",DURSLEEP(pri_attribute.sink_rs));
	PRT_DEF("DURCYCLE = %lu .",DURCYCLE(pri_attribute.sink_rs));
	PRT_DEF("MAX_CYCLE_COUNTER = %ld.\n",MAX_CYCLE_COUNTER(pri_attribute.sink_rs));
	...
	PRT_DEF("MAX_ACTIVE_CYCLES = %u\n",MAX_ACTIVE_CYCLES);
	PRT_DEF("RCV_GRADE_NUM_TO_SET = %u\n",RCV_GRADE_NUM_TO_SET);
	PRT_DEF("PERIODIC_CYCLES = %d\n",PERIODIC_CYCLES);
	...
	PRT_DEF("GEN_DATA_IN_R = %u.\n",GEN_DATA_IN_R);
#if DATA_TX_SUPPORT
	PRT_DEF("CYCLES_PER_PACKET = %u.\n",CYCLES_PER_PACKET(pri_attribute.sink_rs)); 
...
//#endif    
	...
	/*3348		36.99		5.0*/	 
	...
	#else
		if(linkaddr_node_addr.u8[1]==1)// micaz 1.0
		{
			pri_attribute.assigned_id=ID_FOR_MICAZ_ONE; 
			linkaddr_node_addr.u8[0]=1;// 1: sink address
			linkaddr_node_addr.u8[1]=0;
		}else 
		...
		if(linkaddr_node_addr.u8[1]==4)
		{
			pri_attribute.assigned_id=ID_FOR_MICAZ_FOUR;
		}else 
		if(linkaddr_node_addr.u8[1]==5)
		{
			pri_attribute.assigned_id=ID_FOR_MICAZ_FIVE;
		}
	#endif
...
static uint32_t pri_grade_proc(char node_state, uint32_t sender_state_dur)
{
	...
	t = sender_state_dur - (sender_state_dur/DURCYCLE(pri_attribute.sink_rs))*DURCYCLE(pri_attribute.sink_rs);
	
	if(node_state == 'R')
	{	
		if(t < DURSLEEP(pri_attribute.sink_rs))
		{
			pri_attribute.state = 'S';
			state_dur = t;
		...
		{
			pri_attribute.state = 'R';
			state_dur = t - DURSLEEP(pri_attribute.sink_rs);
			//state_period = DURSLOT(pri_attribute.sink_rs);
		}
	}else if(node_state == 'S') 
	{
		if(t < (DURSLEEP(pri_attribute.sink_rs)-2*DURSLOT(pri_attribute.sink_rs)))
		{
			pri_attribute.state = 'S';
			state_dur = t + DURSLOT(pri_attribute.sink_rs); 
			//state_period = DURSLEEP;
		}else if(t < (DURSLEEP(pri_attribute.sink_rs)-DURSLOT(pri_attribute.sink_rs)))
		{
			pri_attribute.state = 'R';
			state_dur = t - (DURSLEEP(pri_attribute.sink_rs)-2*DURSLOT(pri_attribute.sink_rs)); 
			//state_period = DURSLOT(pri_attribute.sink_rs);
		}else 
		{
			pri_attribute.state = 'S';
...
/*---------------------------------------------------------------------------*/
static void 
pri_gdps(rtimer_clock_t receiving_pkt_time, int8_t msg_grade, char msg_state, rtimer_clock_t msg_state_dur)//grade division and pipeline scheduling
{
...
*/
	...
	/*************************************************/	
	rtimer_clock_t sender_state_dur, state_dur;
	uint32_t state_period; 
	sender_state_dur = msg_state_dur+PRI_TM(pri_attribute.sink_rs);
	rtimer_clock_t now;
	...
	pri_show_led();
	now = RTIMER_NOW();
	printf("state_dur=%u, state_period=%u, now=%u, receiving time=%u\n",state_dur,pri_state_period(),now,receiving_pkt_time);
	
	/*No normalization: */
	//state_period = pri_state_period() - (state_dur + (now - receiving_pkt_time));
	/*Normalization for cross-platform support*/
	state_period = pri_state_period() - (state_dur + rtimer_normalization(now - receiving_pkt_time));
	printf("2. state_period=%lu.\n",state_period);
	//watchdog_stop(); 
	if(state_period > RTMAXTIMER)
	{
...
pri_input_packet(void)
{
	...
	if(pri_attribute.ADCSC_unregular_wakeup == 1)//it's an unregular wakeup
	{
		...
		/*Normalization for cross-platform support*/
		pri_control.node_state_dur= rtimer_normalization(receiving_pkt_time - pri_attribute.ADCSC_state_start);
	}else
	{
#endif
	/*No Normalization*/
	...
	/*Normalization for cross-platform support*/
	pri_control.node_state_dur= rtimer_normalization(receiving_pkt_time - pri_attribute.state_start);
...
*/  
		else /*hdr_ptr->dispatch==DISPATCH*/
		{
			...
			if(hdr_ptr->type == TYPE_GRADE)
			{
				...
				#if CROSS_PLATFORM_SUPPORT					
					//state_dur = (uint16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)grade_msg.state_dur)/grade_msg.sink_rs);

					//DURSLOT(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*grade_msg.durslot)/grade_msg.sink_rs;
					//DURSLEEP(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*((SF+1)*grade_msg.durslot))/grade_msg.sink_rs; 
					//pri_attribute.durcycle = ((uint32_t)RTIMER_SECOND*((SF+2)*grade_msg.durslot))/grade_msg.sink_rs; 
					pri_attribute.sink_rs = grade_msg.sink_rs;
					//printf("pri_attribute: durslot: %lu, dursleep: %lu, durcycle: %lu.\n",DURSLOT(pri_attribute.sink_rs),DURSLEEP(pri_attribute.sink_rs),pri_attribute.durcycle);
				#endif
			...
			else if (hdr_ptr->type == TYPE_RTS)
			{
			...
			#endif
				{
					...
					#if CROSS_PLATFORM_SUPPORT
						//state_dur = (uint16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)rts_msg.state_dur)/rts_msg.sink_rs);
						
						//DURSLOT(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*rts_msg.durslot)/rts_msg.sink_rs;
						//DURSLEEP(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*((SF+1)*rts_msg.durslot))/rts_msg.sink_rs; 
						//pri_attribute.durcycle = ((uint32_t)RTIMER_SECOND*((SF+2)*rts_msg.durslot))/rts_msg.sink_rs; 
						pri_attribute.sink_rs = rts_msg.sink_rs;
						//printf("pri_attribute: durslot: %lu, dursleep: %lu, durcycle: %lu.\n",DURSLOT(pri_attribute.sink_rs),DURSLEEP(pri_attribute.sink_rs),pri_attribute.durcycle);
					#endif
				...
				}else if(pri_control.is_receiving_data) //in 'R' state
				{
					if(!pri_control.rcvd_rts && rts_msg.grade == pri_attribute.grade+1 && pri_attribute.rcvd_cts_after_rts)//received a RTS in 'R' state from the right grade
					{
						...
						#if 0//CROSS_PLATFORM_SUPPORT
							offset_or_drift = rts_msg.clock_drift>=0? (int16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)rts_msg.clock_drift)/rts_msg.sink_rs):
								-(int16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)(-rts_msg.clock_drift))/rts_msg.sink_rs);
			...
			//printf("rcvd cts state_dur: %u, arch second: %lu, dur_slot: %lu.\n", cts_msg.state_dur, cts_msg.sink_rs, cts_msg.durslot);

			#if 0//CROSS_PLATFORM_SUPPORT					
				state_dur = (uint16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)cts_msg.state_dur)/cts_msg.sink_rs);
				printf("rcvd cts. state_dur=%u cts state dur=%u.\n\n",state_dur,cts_msg.state_dur);
			#else
				state_dur = cts_msg.state_dur;
			#endif

				pri_control.msg_state_dur = state_dur+PRI_TM(pri_attribute.sink_rs);
				//pri_control.msg_state_dur = TRANSFER_RTIMER_SECOND(cts_msg.state_dur, cts_msg.local_rtimer_second)+PRI_TM(pri_attribute.sink_rs);
			...
			#endif
				{
					...
					#if CROSS_PLATFORM_SUPPORT					
						//DURSLOT(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*cts_msg.durslot)/cts_msg.sink_rs;
						//DURSLEEP(pri_attribute.sink_rs) = ((uint32_t)RTIMER_SECOND*((SF+1)*cts_msg.durslot))/cts_msg.sink_rs; 
						//pri_attribute.durcycle = ((uint32_t)RTIMER_SECOND*((SF+2)*cts_msg.durslot))/cts_msg.sink_rs; 
						pri_attribute.sink_rs = cts_msg.sink_rs;
						//printf("pri_attribute: durslot: %lu, dursleep: %lu, durcycle: %lu.\n",DURSLOT(pri_attribute.sink_rs),DURSLEEP(pri_attribute.sink_rs),pri_attribute.durcycle);
					#endif
				...
				}else if(pri_control.is_sending_data) //received a CTS in 'T' state
				{
					...
					}else if(!pri_control.rcvd_cts)//after sending an RTS before receiving CTS
					{
						...
						#endif
						#if 0//CROSS_PLATFORM_SUPPORT
							offset_or_drift = cts_msg.time_offset>=0? (int16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)cts_msg.time_offset)/cts_msg.sink_rs):
											-(int16_t)(((uint32_t)RTIMER_SECOND*(uint32_t)(-cts_msg.time_offset))/cts_msg.sink_rs);
...
/*---------------------------------------------------------------------------*/
const struct rdc_driver primac_board_driver = {
  "PRIMAC",
  init,
  pri_qsend_packet,
  pri_qsend_list,
