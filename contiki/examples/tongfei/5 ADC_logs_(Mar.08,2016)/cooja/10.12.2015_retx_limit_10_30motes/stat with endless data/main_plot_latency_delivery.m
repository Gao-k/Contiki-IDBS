...
for ii = file_num
    ...
    [~,~,~,~,~,~,~,~,grade,~,latency,~,~,~,~ ]=textread(file_name,'%s %s %d %s %s %s %d %s %d %s %d %s %d %s %s');
...
AX = legend([adc_18_fa_latency adc_18_nfa_latency pdc_18_nfa_latency ], ...
    'ADC-FA','ADC-No FA','PDC',1);%设置legend位置。
...
AX = legend([adc_18_fa_delivery adc_18_nfa_delivery pdc_18_nfa_delivery], ...
'ADC-FA','ADC-No FA','PDC',4);%设置legend位置。
LEG = findobj(AX,'type','text');
axis([10 50 0 1.001]);
set(LEG,'FontSize',11);%设置legend字体大小
set(gca,'FontSize',16);%设置坐标字体大小
