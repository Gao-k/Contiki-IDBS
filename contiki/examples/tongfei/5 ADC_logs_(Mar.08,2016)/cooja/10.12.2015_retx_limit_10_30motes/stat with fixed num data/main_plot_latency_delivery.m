...
total_data_num = 24*15; % 24 nodes, 15 pkts/node generated
output_data_folder = 'R_data';
data_format = '%s %s %d %s %s %s %d %d %d';
for ii = file_num
    %% adc sf =18, with free addressing
    file_name = [output_data_folder,'/adc_18_fa_',int2str(ii*10),'s.txt'];
    ...
    file_name = [output_data_folder,'/adc_18_nfa_',int2str(ii*10),'s.txt'];
    [~,~,~,~,~,~,grade,latency,~]=textread(file_name,data_format);
    ...
    %% pdc sf=18
    file_name = [output_data_folder,'/pdc_18_nfa_',int2str(ii*10),'s.txt'];
    [~,~,~,~,~,~,grade,latency,~]=textread(file_name,data_format);
    ...
    %% pdc sf=18
    file_name = [output_data_folder,'/pdc_10_nfa_',int2str(ii*10),'s.txt'];
    [~,~,~,~,~,~,grade,latency,~]=textread(file_name,data_format);
...
l_adc_18_fa = 'b-s'; l_adc_18_nfa = 'b-^'; l_pdc_18_nfa = 'b->';
l_adc_10_fa = 'r--s'; l_adc_10_nfa = 'r--^'; l_pdc_10_nfa = 'r-->';
linewidth_value=2.5;
%semilogy
...
AX = legend([adc_10_fa_latency adc_10_nfa_latency pdc_10_nfa_latency adc_18_fa_latency adc_18_nfa_latency pdc_18_nfa_latency ], ...
    'ADC-FA (SL=10)','ADC-No FA (SL=10)','PDC (SL=10)','ADC-FA (SL=18)','ADC-No FA (SL=18)','PDC (SL=18)',1);%设置legend位置。
...
AX = legend([adc_10_fa_delivery adc_10_nfa_delivery pdc_10_nfa_delivery adc_18_fa_delivery adc_18_nfa_delivery pdc_18_nfa_delivery], ...
'ADC-FA (SL=10)','ADC-No FA (SL=10)','PDC (SL=10)','ADC-FA (SL=18)','ADC-No FA (SL=18)','PDC (SL=18)',4);%设置legend位置。
LEG = findobj(AX,'type','text');
axis([10 50 0 1.001]);
set(LEG,'FontSize',11);%设置legend字体大小
set(gca,'FontSize',16);%设置坐标字体大小
