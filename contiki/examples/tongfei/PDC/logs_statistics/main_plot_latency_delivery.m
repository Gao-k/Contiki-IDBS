...
seed_array = [123462 123477 123482];%[123477 123482 123487];

total_data_num = 30*20; % 30 nodes, 20 pkts/node generated

data_format = '%s %s %d %s %s %s %d %d %d';
sim_i=1;
for seed = seed_array
    output_data_folder = ['.\logs_fixed_data_',int2str(seed),'\R_data']; %'R_data';
...
'ADC-FA (SSL=10)','ADC-No FA (SSL=10)','ADC-FA (SSL=18)','ADC-No FA (SSL=18)','PDC (SSL=10)','PDC (SSL=18)',4);%设置legend位置。
LEG = findobj(AX,'type','text');
axis([10 50 0 1.001]);
set(LEG,'FontSize',11);%设置legend字体大小
set(gca,'FontSize',16);%设置坐标字体大小
