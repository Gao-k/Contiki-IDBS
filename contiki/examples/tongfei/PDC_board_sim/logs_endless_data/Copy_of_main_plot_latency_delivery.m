...
total_data_num = 4*90; % 4 nodes, 90 pkts/node generated
output_data_folder = 'R_data';
data_format = '%d %s %d %s %s %s %d %d %d';
for ii = file_num
...
AX = legend([ adc_10_fa_latency    adc_10_nfa_latency ], ...
   'ADC (SL=10)','ADC-FA (SL=10)',0);%设置legend位置。
...
AX = legend([ adc_10_fa_delivery    adc_10_nfa_delivery], ...
'ADC (SL=10)','ADC-FA (SL=10)',4);%设置legend位置。
LEG = findobj(AX,'type','text');
% axis([0 10 0 1.1]);
set(LEG,'FontSize',11);%设置legend字体大小
set(gca,'FontSize',16);%设置坐标字体大小
