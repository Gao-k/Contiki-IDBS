	...
	foreach SF($SF_list)
		...
		foreach FAORNOT($FAORNOT_list)
			...
			foreach PGI($PGI_list)
				...
				gnome-terminal -x  bash -c "make hello-world.upload TARGET=micaz PORT=/dev/ttyUSB4; cat < /dev/ttyUSB4 2>&1 | tee $LOGSDIR/$CURRENTFOLDER/m2.txt; exec bash"
				sleep 13s
